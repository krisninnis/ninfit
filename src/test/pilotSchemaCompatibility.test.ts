...
describe('local-first pilot evidence — schema/backward compatibility', () => {
  it('normalises an older v1 backup that predates metricSamples', () => {
    ...
    const legacy = structuredClone(envelope) as Omit<typeof envelope, 'data'> & {
      data: Omit<typeof envelope.data, 'metricSamples'> & {
        metricSamples?: typeof envelope.data.metricSamples;
      };
    };
